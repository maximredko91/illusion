...
class TmdbClient(private val apiKeyProvider: () -> String = { BuildConfig.TMDB_API_KEY }) {
    ...
    /** False if no key has been entered in-app or set via `illusion.tmdb.apiKey` in local.properties - callers should disable the add-media entry point rather than let every request fail. */
    ...
    private suspend fun get(path: String, params: HttpUrl.Builder.() -> Unit): String = withContext(Dispatchers.IO) {
        check(isConfigured) { "TMDB API key not configured - set illusion.tmdb.apiKey in local.properties" }
